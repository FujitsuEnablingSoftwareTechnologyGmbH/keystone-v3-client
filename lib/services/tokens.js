...
utils.mixin(TokenService.prototype, {
   ...
   * - false valud if token is not valid
   *
  ...
  /**
   * Refreshes token using information provided in it such as:
   * - username and password
   *
   * Returned promises resolves successfully only if token has been successfully
   * refreshed and new token for given username has been saved. Resolved promises
   * returns new token
   *
   * Returned promise may be rejected because:
   ...
   * @param subjectToken token to be evaulated
   *
   * @returns {Promise} new token (resolve) / failed refreshment reason(reject)
   */
...
function refreshToken(token) {
  ...
  function doRefreshToken(token) {
    ...
    function tokenRefreshed(data) {
      ...
      tokenCache.put(newToken, tokenData, refreshToken.bind(self));
